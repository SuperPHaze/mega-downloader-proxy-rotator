...
- **Tabbed interface** with a compact 3-zone dashboard (speed, downloads, proxy) — a speed sparkline, a segmented bar for download status, compact cards for proxy pool status — button-based job filters, light/dark theme, global and per-job pause/resume/cancel.
