...
If the program is closed (or crashes) with downloads not yet completed, on the next launch it offers to **reload the remaining links** ("Riprendi sessione precedente?" — Resume previous session?). By accepting, the links return to the list and — thanks to the resume described below — restart from the pieces already downloaded: just press Start.
